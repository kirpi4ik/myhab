	...
	<q-card flat class="solar-plant-card">
		...
		<q-card-section class="header-section q-pa-md">
			<div class="row items-center justify-between">
				...
				<div class="col-auto row q-gutter-xs items-center">
					<!-- Device Status Badge -->
					<q-chip :color="deviceStatusColor" text-color="white" size="sm" :icon="deviceStatusIcon">
						{{ deviceStatusLabel }}
					</q-chip>
					<!-- System Health Badge -->
					<q-chip
					...
					>
						{{
							systemHealth === 3
								? $t('solar.status_ok')
								: systemHealth === 2
									? $t('solar.status_warning')
									: $t('solar.status_error')
						}}
					</q-chip>
					<!-- Reports Button -->
		...
		<q-card-section class="realtime-section q-pa-md">
			...
			<div class="power-flow-container q-mb-lg">
				<div class="flow-diagram">
					...
					<svg class="flow-lines" viewBox="0 0 400 300" preserveAspectRatio="xMidYMid meet">
						...
						<polygon
							points="115,170 126,162 115,157"
							class="flow-arrow"
							:class="{ active: solarProductionWatts > 0 && totalHouseConsumptionWatts > 0 }"
						/>

						<!-- PV to Grid -->
						<line x1="250" y1="100" x2="285" y2="170" class="flow-line pv-to-grid" stroke-width="3" />
						<polygon
							points="285,170 274,162 285,157"
							class="flow-arrow"
							:class="{ active: solarProductionWatts > 0 && meterActivePowerWatts > 0 }"
						/>
			...
			<div class="phases-container">
				<div class="text-caption text-grey-7 text-uppercase q-mb-sm">{{ $t('solar.production_now') }}</div>
	...
	setup(props) {
		...
		const loadDetails = () => {
			...
			Promise.all([
				...
				const allPorts = [...inverterData.data.device.ports, ...meterData.data.device.ports];
