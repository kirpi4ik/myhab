<template>
  ...
  <q-dialog v-model="visibleNSc" transition-show="jump-up" transition-hide="jump-down" class="heat-scheduler" :maximized="$q.platform.is.mobile?visibleNSc:false">
    <q-card class="bg-white" :style="$q.platform.is.mobile?'':'width: 500px; max-width: 80vw;'">
      ...
      <q-card-section align="around" class="scroll">
        ...
        <div class="text-grey-10">
          ...
          <q-btn label="Add" class="bg-green-5" icon="mdi-plus" @click="addListItemConfig(zone.id, 'key.temp.schedule.list.value', heatScheduler.time, heatScheduler.temp)"/>
        </div>
      </q-card-section>
      <q-card-section align="around" class="scroll">
