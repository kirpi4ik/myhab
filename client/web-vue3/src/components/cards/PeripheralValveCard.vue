<template>
  <q-card
    class="peripheral-valve-card text-white"
    :class="{ 'valve-open': isValveOpen, 'valve-closed': !isValveOpen }"
  >
    <!-- Status Indicator Badge -->
    <div class="status-indicator" :class="{ 'indicator-on': isValveOpen, 'indicator-off': !isValveOpen }">
      <q-icon :name="isValveOpen ? 'mdi-water' : 'mdi-water-off'" size="16px" class="q-mr-xs"/>
      <span class="text-caption text-weight-medium">{{ isValveOpen ? $t('valve_card.open') : $t('valve_card.closed') }}</span>
    </div>
    ...
    <q-item class="card-content">
      ...
      <q-item-section avatar>
        <q-avatar
          size="60px"
          class="valve-avatar"
          :class="isValveOpen ? 'avatar-on' : 'avatar-off'"
        >
          <q-icon
            :name="isValveOpen ? 'mdi-valve-open' : 'mdi-valve-closed'"
            :color="isValveOpen ? 'light-blue-1' : 'blue-grey-2'"
            size="40px"
          />
          <div v-if="isValveOpen" class="valve-glow"></div>
        </q-avatar>
      </q-item-section>

      <!-- Valve Info -->
      <q-item-section>
        <q-item-label class="text-weight-medium text-h5 valve-name">
          {{ asset.data?.name || 'Unknown Valve' }}
        ...
        <q-item-label v-if="asset.data?.description" class="text-weight-light text-blue-grey-2 valve-description">
        ...
        <q-item-label v-if="timeoutConfig || showExpiration" class="q-mt-xs">
          <q-chip
            size="sm"
            :color="isValveOpen ? 'cyan-8' : 'blue-grey-8'"
            text-color="white"
          ...
          >
            <q-list>
              ...
              <q-item clickable v-close-popup @click="shareDialogVisible = true">
                ...
                <q-item-section>
                  <q-item-label>{{ $t('valve_card.share') }}</q-item-label>
                </q-item-section>
              </q-item>
              ...
              >
                ...
                <q-item-section>
                  <q-item-label>{{ $t('valve_card.details') }}</q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
          </q-btn-dropdown>
    ...
    <q-separator :class="isValveOpen ? 'separator-glow' : ''" />

    <!-- Toggle Switch -->
    <q-card-section class="toggle-section">
      <div class="toggle-container">
        <div class="toggle-wrapper">
          <div class="toggle-label-wrapper toggle-label-off-wrapper" :class="{ 'active': !isValveOpen }">
            <q-icon name="mdi-valve-closed" size="16px"/>
            <span class="label-text">{{ $t('valve_card.closed') }}</span>
          </div>
        ...
        <toggle
          ...
          <div class="toggle-label-wrapper toggle-label-on-wrapper" :class="{ 'active': isValveOpen }">
            <q-icon name="mdi-valve-open" size="16px"/>
        ...
        <div v-if="isDeviceOffline" class="offline-message">
          ...
          {{ $t('valve_card.device_offline') }}
        </div>
      </div>
    </q-card-section>

    <!-- Bottom Accent Bar -->
    <div class="accent-bar" :class="{ 'accent-active': isValveOpen }"></div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
     ...
     */
    const isValveOpen = computed(() => {
      const portValue = asset.value?.data?.connectedTo?.[0]?.value;
      return portValue === 'ON';
    });

    /**
     * Get valve state for toggle (read-only)
     * Reads from connectedTo[0].value which contains "ON" or "OFF"
     */
    const valveState = computed(() => {
      const portValue = asset.value?.data?.connectedTo?.[0]?.value;
      return portValue === 'ON';
    });
    ...
    const showExpiration = computed(() => {
      return isValveOpen.value && !!expirationTime.value;
    });
...
.peripheral-valve-card {
  ...
  // Closed state (OFF) - calm slate background
  ...
  // Open state (ON) - water, flowing background
  &.valve-open {
