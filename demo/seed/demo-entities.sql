...
-- Tuya water valve
--
-- A TUYA-model device: state and commands travel in the myhab/tuya/ sub-namespace
-- (MQTTTopic.TUYA), which the simulator answers via the per-device "prefix" in
-- devices.json. In a real installation the bridges/tuya container plays that
-- role against the physical valve.
-- --------------------------------------------------------------------------
INSERT INTO device_controllers (id, version, code, name, description, model, status,
                                ts_created, ts_updated, en_type)
VALUES
  (3005, 0, 'tuya_valve_garden', 'Garden water valve', 'Tuya WiFi ball valve on the garden supply line', 'TUYA', 'ONLINE', now(), now(), 'DEVICE');

INSERT INTO device_ports (id, version, device_id, internal_ref, name, type, state, value,
                          ts_created, ts_updated, en_type)
VALUES
  (4022, 0, 3005, 'valve', 'Garden water valve', 'SWITCH', 'ACTIVE', 'OFF', now(), now(), 'PORT');

INSERT INTO device_peripherals (id, version, name, description, category_id, max_amp,
                                ts_created, ts_updated, en_type)
VALUES
  (5022, 0, 'Garden Water Valve', 'Shut-off valve for the garden water supply', 2006, 0.5, now(), now(), 'PERIPHERAL');
